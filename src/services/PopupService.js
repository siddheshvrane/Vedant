...
class PopupServiceClass {
    ...
    hide() {
        ...
        if (this._confirmationRejecter) {
            this._confirmationRejecter(new Error('Confirmation dismissed by user or system.'));
    ...
    showConfirmation(message, title = 'Confirm Action', confirmText = 'Confirm', cancelText = 'Cancel') {
        ...
        if (this._confirmationRejecter) {
            this._confirmationRejecter(new Error('New confirmation dialog opened before previous one was resolved.'));
        }
